 ...
 */
export async function generateZKProof(files: File[], qualityScore: number): Promise<ZKProof> {
...
export async function verifyZKProof(zkProof: ZKProof): Promise<{
  ...
  const [fileSize, fileCount, qualityScore] = zkProof.publicSignals.map((s) => parseInt(s, 10));
